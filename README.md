项目简介：使用前沿NLP技术，搭建一套完整的智能医生系统，最终通过微信公众号为用户提供服务。

在线部分:项目上线在了微信公众号（杨靳言先）可关注体验
使用Flask web服务框架和Gunicorn组件，在线部分包括werobot服务模块、主要逻辑服务模块、句子相关模型服务模块、会话管理模块（使用Redis）、Neo4j图数据库模块及规则对话/百度Unit模块。
数据流：用户请求通过werobot服务进入主服务，调用Redis、句子相关模型和Neo4j图数据库，最终将结果发送给对话规则模板或通过Unit API回复。

离线部分:包括结构与非结构化数据采集模块、NER模型（BiLSTM+CRF）及实体审核模型（bert-base-chinese+Fine-tuning）。
数据流：结构化数据直接审核并写入图数据库，非结构化数据通过NER模型抽取实体，经审核后再写入图数据库。
